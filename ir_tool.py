#!/usr/bin/env python3
"""Broadlink RM4 IR helper (multi-device aware). Usage:
  ir_tool.py discover
  ir_tool.py use   <host>          # pick which RM4 is the active blaster
  ir_tool.py learn <name> [host]   # learn a code (optionally via a specific RM4)
  ir_tool.py send  <name> [host]   # send a code (optionally via a specific RM4)
  ir_tool.py list
"""
import sys, os, json, time, base64
import broadlink

STORE   = "/usr/local/etc/ir_codes.json"
DEVFILE = "/usr/local/etc/ir_device.json"
...
def get_device(host=None, save=True):
    ...
    if len(devs) > 1:
        ...
        sys.exit("Multiple RM4s found - pick one with 'ir_tool.py use <host>':\n" + lines)
...
def cmd_learn(name, host=None):
    ...
    print(">> Aim your ORIGINAL remote at RM4 %s and press the '%s' button now (10s)..." % (dev.host[0], name))
    packet = None
    for _ in range(10):
        time.sleep(1)
        ...
        if packet: break
    if not packet: sys.exit("Timed out - no IR captured. Right RM4? Aim closer.")
